...
void SummaryStrengtheningInfo::update(const int& num_coeffs, const int total_num_cuts) {
  this->num_str_affected_cuts += (num_coeffs > 0);
  updateStatsBeforeFinalize(this->num_coeffs_strengthened, num_coeffs, total_num_cuts);
} /* SummaryStrengtheningInfo::update */

void SummaryStrengtheningInfo::finalize(const int total_num_cuts) {
  finalizeStats(this->num_coeffs_strengthened, total_num_cuts);
} /* SummaryStrengtheningInfo::finalize */

void SummaryStrengtheningInfo::unfinalize(const int prev_num_cuts) {
  unfinalizeStats(this->num_coeffs_strengthened, prev_num_cuts);
} /* SummaryStrengtheningInfo::unfinalize */
...
const int countGapInfoEntries = 10;
...
void printHeader(const StrengtheningParameters::Parameters& params,
  ...
  { // GAP INFO
    ...
    fprintf(logfile, "%s%c", "MYCUTS % GAP CLOSED", SEP); count++; // 4
    fprintf(logfile, "%s%c", "GMIC+MYCUTS % GAP CLOSED", SEP); count++; // 5
    fprintf(logfile, "%s%c", "UNSTR MYCUTS % GAP CLOSED", SEP); count++; // 6
    fprintf(logfile, "%s%c", "UNSTR GMIC+MYCUTS % GAP CLOSED", SEP); count++; // 7
    fprintf(logfile, "%s%c", "RCVMIP MYCUTS % GAP CLOSED", SEP); count++; // 8
    fprintf(logfile, "%s%c", "RCVMIP GMIC+MYCUTS % GAP CLOSED", SEP); count++; // 9
    fprintf(logfile, "%s%c", "RCVMIP ALL % GAP CLOSED", SEP); count++; // 10
...
void printCertificateInfo(
  ...
  for (const auto& info : info_vec) {
    ...
    StatVector iter_stats = computeStats(info.num_iterations);
    ...
    StatVector time_stats = computeStats(info.rcvmip_time);
